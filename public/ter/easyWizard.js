...
$.fn.wizard = function(config) {
  ...
  var exitText = config.exit || 'Close';
  var backText = config.back || 'ฺBack';
  var nextText = config.next || 'Next';
  var finishText = config.finish || 'Sign Up';
  ...
  var validateFinish = config.validateFinish || function(){return true;};
    ...
    btns += '<button type="button" class="btn btn-primary wizard-button-finish" '+contentForModal+'  id="Signup">'+ finishText +'</button>';
