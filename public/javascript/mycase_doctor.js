(function(){
    ...
    firebase.auth().onAuthStateChanged(user=>{
        if(user){
            const type = firebase.database().ref('alluser/'+user.uid).orderByChild('id');
            type.once('value',typesu=> {
                const typeuser= firebase.database().ref().child('case/'+typesu.val().id);
                if(typesu.val().who=="doctor"){
                ...
                for (i = 0; i < userpatient.val().length; i++) {
                    ...
                    dataprofile.on('value', patient=>{    
                       ...
                       document.querySelector("#data").innerHTML += profile;
                      $("#loader").hide();
                    });
                }
...
function viewdata(key){
    localStorage.setItem("keyView",key);
